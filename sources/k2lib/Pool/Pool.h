...
private:
  ...
  static const size_t NO_BYTES =
      ...
      BITS_IN_UINT8; //!< Required number of bytes to hold information about
                     //!< free/allocated pool objects.
  ...
  bool testBit(int bit_index)
  {
    ...
    return (0 != (*((uint8_t *)(&info[0] + byte_offset)) &
                  (uint8_t)(1 << bit_index)));
  }
...
template <typename T, int SIZE> Pool<T, SIZE>::Pool()
{
...
template <typename T, int SIZE> T *Pool<T, SIZE>::palloc()
{
...
template <typename T, int SIZE> void Pool<T, SIZE>::free(T *p)
{
...
template <typename T, int SIZE> int Pool<T, SIZE>::size()
{
